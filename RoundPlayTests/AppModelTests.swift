...
/// A private, disposable `UserDefaults` domain — never `.standard`.
///
/// `RoundPlayTests` is an app-hosted target: `xcodebuild test` launches the real `RoundPlay.app`
/// on the simulator to run these tests, so `UserDefaults.standard` inside a test *is* that
/// simulator's real, persistent app defaults — the same store a manually-onboarded "me" player's
/// id lives in. A test that clears `MyPlayer.defaultsKey` on `.standard` for isolation wipes that
/// real pointer, and the next real solo round self-heals by minting a second "Me" behind it —
/// exactly how this app ended up with duplicate "Me" roster entries during development. Every
/// test that touches `MyPlayer` must inject one of these instead.
private func isolatedDefaults() -> UserDefaults {
    UserDefaults(suiteName: "AppModelTests.\(UUID().uuidString)")!
}

/// Grouped for readability — no longer for isolation. Before `defaults` injection existed, these
/// tests raced on the single shared `UserDefaults.standard` key and needed `.serialized` to avoid
/// stepping on each other; each now gets its own private suite, so they run fully in parallel
...
struct SoloMyPlayerTests {
    ...
    func soloUsesFullHandicap() throws {
        ...
        let model = NewRoundModel()
        model.course = course
        model.makeSolo(in: context, defaults: isolatedDefaults())
    ...
    func soloSkipsGroupOnlySteps() throws {
        ...
        solo.makeSolo(in: context, defaults: isolatedDefaults())
    ...
    func soloSeatResolvesToMe() throws {
        ...
        let created = MyPlayer.resolve(in: context, defaults: defaults)
        #expect(created.name == "Me")
        #expect(defaults.string(forKey: MyPlayer.defaultsKey) == created.id.uuidString)

        // Stored and present: the same record comes back, not a second one.
        #expect(MyPlayer.resolve(in: context, defaults: defaults).id == created.id)

        // Stored but deleted from the roster: a fresh record replaces it.
        context.delete(created)
        try context.save()
        let replacement = MyPlayer.resolve(in: context, defaults: defaults)
    ...
    func soloNetDiffersFromGross() throws {
        ...
        context.insert(me)
        let defaults = isolatedDefaults()
        defaults.set(me.id.uuidString, forKey: MyPlayer.defaultsKey)

        let model = NewRoundModel()
        model.course = course
        model.makeSolo(in: context, defaults: defaults)
        let round = try #require(model.makeRound(in: context))
